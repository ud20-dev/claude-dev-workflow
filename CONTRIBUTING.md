...
### Versioning : le README ne bouge jamais de numéro pendant un WIP
- Décision : `> Version X.Y.Z` en haut du README reste toujours la dernière version **réellement publiée** — identique à `docs/CLAUDE.md`. Tout ce qui est en cours de construction vit uniquement dans `CHANGELOG.md` > `[Unreleased]`. Les deux lignes de version (README + `docs/CLAUDE.md`) ne bougent que dans le même commit, celui qui publie vraiment la version
- Pourquoi : le hook `SessionStart` de chaque projet installé compare sa version locale au premier numéro `X.Y.Z` trouvé dans le README publié sur GitHub — sans comprendre un texte d'avertissement à côté. Une version précédente de ce README affichait `1.2.0 (WIP, do not use...)` pendant que `docs/CLAUDE.md` était encore à `1.1.0` ; résultat observé en pratique : le hook a annoncé "mise à jour disponible : 1.1.0 → 1.2.0" alors que cette version n'était pas prête. Un utilisateur qui aurait accepté aurait migré vers du travail en cours, pas une version stable
- Alternative refusée : garder le marqueur `(WIP, do not use...)` et apprendre au hook à le reconnaître pour ne pas se déclencher
- Pourquoi refusée : rustine sur un symptôme — il suffirait d'oublier une fois de mettre à jour la regex ou le texte exact du marqueur pour que le bug revienne. Ne jamais afficher un numéro de version qui n'est pas encore sûr élimine la classe de bug entière, pas seulement ce cas précis

### `apply-update.sh` : automatiser seulement la partie sans jugement de la mise à jour
- Décision : nouveau script qui clone le template et copie tout fichier entièrement absent localement dans `.claude/hooks/`, `.claude/scripts/`, `.claude/SKILLS/`, `.github/workflows/` et `docs/` — jamais un fichier déjà présent. Ne touche ni `docs/CLAUDE.md` (ajout de ligne dans une table existante) ni `.claude/settings.json` (ajout de clé dans un JSON existant) : ces deux-là restent comparés à la main par Claude, seuls endroits où la mise à jour ajoute du contenu *à l'intérieur* d'un fichier qui existe déjà plutôt que tout un fichier neuf
- Pourquoi : la procédure de mise à jour demandait à Claude de comparer six emplacements différents (`docs/CLAUDE.md`, `.claude/hooks/`, `.claude/scripts/`, `.claude/SKILLS/`, `.github/workflows/`, `.claude/settings.json`) un par un, par lecture et raisonnement — alors que quatre d'entre eux suivent une règle 100% mécanique déjà appliquée par `install.sh` sur un projet neuf : "fichier absent → copier, fichier présent → ne jamais toucher". Un script fait cette partie en une passe, sans appel d'outil ni raisonnement, laissant à Claude uniquement les deux cas qui exigent vraiment un jugement (une ligne ou une clé peut avoir été reformulée/ajoutée volontairement, un fichier entier ne peut pas)
- Alternative refusée : tout automatiser, y compris `docs/CLAUDE.md` et `settings.json`, avec un diff plus intelligent
- Pourquoi refusée : la règle "ne jamais modifier une ligne déjà présente même si sa formulation diffère" (voir la décision de mise à jour additive plus haut) demande de distinguer une ligne oubliée d'une ligne volontairement reformulée — un jugement sémantique, pas quelque chose qu'un diff textuel peut trancher en sécurité sans risquer d'écraser une adaptation faite pendant le projet
- Testé : simulation d'un projet resté en 1.1.0 (avant le split, avant les skills, avant le workflow) — le script a ajouté exactement les fichiers manquants et n'a touché à aucun fichier déjà présent (vérifié après coup)

### Hook `PreToolUse` sur Write/Edit : transformer un rappel textuel en rappel visible au bon moment
- Décision : nouveau `.claude/hooks/pre-tool-use.sh`, câblé sur Write/Edit dans `settings.json`, qui inspecte le chemin du fichier touché et affiche un rappel ciblé — chemin contenant `components` → "COMPONENTS.md vérifié ?", fichier de code (`.tsx`/`.ts`/`.py`/`.go`/...) → "mérite une entrée ERRORS.md/CHANGELOG.md ?", chemin de migration/schéma → "DATABASE.md à jour ?". Ne bloque jamais — toujours exit 0, silencieux si rien ne correspond ou si l'entrée est illisible
- Pourquoi : audit réel d'un projet utilisant le template sur plusieurs sessions — `COMPONENTS.md` jamais ouvert avant de proposer 5 nouveaux composants, deux bugs frontend réellement corrigés jamais notés dans `ERRORS.md`, une refonte mobile entière jamais loggée dans `CHANGELOG.md`. Cause identifiée : `settings.json` ne déclarait qu'un hook `SessionStart` — chaque règle de "Règles absolues" ("chaque bug résolu → ERRORS.md immédiatement") reposait à 100% sur l'auto-discipline de l'IA en cours de tâche, sans aucun filet technique entre la règle écrite et l'action réelle
- Alternative refusée : se contenter de renforcer le texte des règles (rappels plus insistants, déclaration explicite de vérification en réponse)
- Pourquoi refusée partiellement, pas totalement : gardé en complément (voir "Règles absolues" — déclaration explicite avant de créer un composant), mais un rappel textuel reste une bonne intention qu'une tâche chargée peut noyer ; un hook s'exécute à chaque Write/Edit sans dépendre de s'en souvenir
- Alternative refusée : bloquer l'écriture (exit non-zéro) si `COMPONENTS.md` n'a pas été lu avant
- Pourquoi refusée : aucun moyen fiable de savoir si le fichier a été lu pendant la session sans état persistant supplémentaire à maintenir ; un faux positif bloquerait une écriture légitime. Un rappel qui n'empêche jamais d'avancer, même mal informé, coûte moins qu'un blocage mal informé
- Testé : entrée JSON réaliste (Write sur `src/components/Button.tsx`, Edit sur `backend/schema.go`) déclenche le bon rappel ; entrée dans `docs/` ne déclenche rien (éditer la doc est déjà l'action attendue) ; entrée vide ou JSON cassé n'affiche rien et ne plante jamais

### Ambiguïté mémoire tranchée, réapplication de l'Aiguillage, rituel de fin de tâche
- Décision : trois ajouts ponctuels à `docs/CLAUDE.md`, tous issus du même audit réel — (1) une ligne explicite disant que `docs/` est la seule mémoire qui compte, un système de mémoire personnel de l'IA externe au repo ne doit jamais contenir d'info spécifique au projet ; (2) une note sous "Aiguillage" précisant que la table se réapplique à chaque nouveau sous-sujet abordé en cours de session, pas seulement à l'ouverture ; (3) une règle "fin de tâche, pas seulement fin de session" pour ERRORS.md/CHANGELOG.md
- Pourquoi : (1) `docs/CLAUDE.md` ne mentionnait jamais l'existence possible d'une mémoire personnelle de l'IA séparée du repo — ambiguïté structurelle, pas une règle mal appliquée puisqu'aucune règle n'existait avant. (2) l'audit montre une session qui bascule de sujet sans repasser par la table de routage. (3) une session peut couvrir plusieurs jours et plusieurs tâches ; attendre la fin de session pour logger un bug corrigé au milieu revient en pratique à ne jamais le faire — observé concrètement (refonte mobile entière jamais loggée)
- Alternative refusée pour (3) : garder uniquement la règle "fin de session", en comptant sur le rappel `PreToolUse` ci-dessus pour compenser
- Pourquoi refusée : le hook rappelle *qu'il faut vérifier* au moment de l'écriture du code, mais rien ne rappelle de le faire *après*, une fois la tâche terminée et l'attention déjà passée à autre chose — les deux mécanismes couvrent des moments différents, pas redondants

### `docs/CLAUDE.md` change rarement, par design — le dire explicitement
- Décision : bandeau ajouté en tête de `docs/CLAUDE.md` — un réflexe/bug/décision/composant ne s'y ajoute jamais, seulement un vrai changement structurel (nouveau fichier, nouvelle route d'aiguillage)
- Pourquoi : retour d'usage — conflits git répétés sur `docs/CLAUDE.md` lors de pull requests, quand un dev frontend et un dev backend y ajoutent chacun une ligne en parallèle. Contrairement à `PROGRESS.md` (édité à chaque session, d'où le split par couche), `docs/CLAUDE.md` est conçu pour être quasi statique après la session 1 — le vrai problème n'est pas le fichier, c'est que du contenu qui a déjà sa maison dédiée (`FEEDBACK.md`, `ERRORS.md`, `DECISIONS.md`, `COMPONENTS.md` — tous conçus pour la concurrence, SENTINEL/LIFO ou séparés par couche) finit copié/reformulé dans l'orchestrateur par réflexe
- Alternative refusée : scinder `docs/CLAUDE.md` par couche comme `PROGRESS.md`
- Pourquoi refusée : `docs/CLAUDE.md` est LA carte partagée — les deux couches ont besoin de voir exactement la même version de l'Aiguillage et de la Structure. Le scinder casserait son rôle même de référence commune, pour un problème qui se résout en clarifiant ce qui n'a pas sa place ici plutôt qu'en dupliquant le fichier

### Contenu spécifique à une stack retiré de `ERRORS.md`/`FEEDBACK.md`, laissé aux presets
- Décision : deux entrées `ERRORS.md` nommant DaisyUI explicitement supprimées (contenu déjà dupliqué mot pour mot dans `frontend/presets/tailwind-daisyui.md` — c'est sa maison naturelle) ; une entrée `FEEDBACK.md` reformulée pour ne plus nommer DaisyUI/Tailwind dans le texte générique
- Pourquoi : `ERRORS.md`/`FEEDBACK.md` sont livrés à **tout** projet qui installe le template, quelle que soit sa stack — `STYLE.md` revendique explicitement d'être "neutre vis-à-vis de la stack" dès sa première ligne, mais deux fichiers voisins avec le même statut de contenu livré-par-défaut contenaient du contenu 100% spécifique à un seul preset parmi trois. Un projet en `css-pur` ou `tailwind-only` héritait d'un bug report inapplicable, exactement le genre de "parasite" que le catalogue de presets existe pour éviter
- Alternative refusée : garder les entrées mais les marquer "applicable seulement si preset tailwind-daisyui"
- Pourquoi refusée : le contenu exact (cause, fix, code CSS) existe déjà tel quel dans `tailwind-daisyui.md`, marquer au lieu de retirer aurait juste ajouté une troisième copie à maintenir en synchro plutôt que d'éliminer la duplication
